...
class WSJ():
    ...
    def test(self):
        if self.test_set is None:
            self.test_set = (np.load(os.path.join(os.environ['WSJ_PATH'], 'AIRBUS_test_features.npy'), encoding='bytes'), None)
...
def main(args, model_dir):
    ...
    test_loader = DataLoader(
        myDataset(wsj.test, STR_DICT, True), shuffle=False,
        batch_size=32, collate_fn=my_collate_fn, **kwargs)
    ...
    beamsearcher = BeamSearcher(args, INPUT_DIM, vocab_size, STRINGS, beam_size=8, eos_index=1)
    ...
    with open(model_dir + '.csv', 'w', newline='') as f:
        ...
        w.writerow(['Id', 'Predicted'])
        id = 0
        for sequence, seq_length, _, _, _ in test_loader:
            if torch.cuda.is_available():
                sequence = sequence.cuda()

            labels, score, beams = beamsearcher(to_variable(sequence), seq_length) # weird
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', dest='output_dir', type=str, default='./input-enhance')
    args = parser.parse_args(sys.argv[1:])

    #grid_search()
    main(args, model_dir=None)
    #main(args, model_dir='./airbus_normalize/trloss_9.751346588134766_vlloss_19.87523078918457_epoch_7batch_size_20_cuda_False_decoder_dim_1024_encoder_dim_256_enhance_dim_256_init_lr_0.001_key_dim_1024_linear_dim_1024_net_out_prob_0.1_value_dim_1024_weight_decay_0')
